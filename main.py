...
def parens_update(current_output, next_input):
    ...
    if next_input == ')':          # if a close paren is detected
        if current_output <= 0:      # if there is not an open before it, error
            return -math.inf
        else:                        # good
            return current_output - 1 #decrement existing count
    elif next_input == '(':            # new open parens increment count
...
def parens_match_dc_helper(mylist):
    ...
    if len(mylist) == 0:
        ...
        return (0, 0)
    elif len(mylist) == 1: #check individual characters at base case
        if mylist[0] == '(':

            return (0, 1) # one unmatched (
        elif mylist[0] == ')':
            
            return (1, 0) # one unmatched )   these are checked when merged
        else:
